...
//antiprivado.js
smsprivado: (m) => `*@${m.sender.split`@`[0]} ESTA PROIBIDO ESCREVE NO PRIVADO DO BOT.*\n\n⚠️ \`\`\`PRIMERA ADVERTENCIA\`\`\` ⚠️`, 
smsprivado1: (m, grupos) => `*@${m.sender.split`@`[0]} SE FOI MENCIONADO ANTERIORMENTE QUE NÃO SE PODE ESCREVER NO PV DO  BOT 🫤*\n\n👇 PODE ENTRA NESTE GRUPO PARA USAR O BOT\n\n${grupos}\n\n*SI VOLTA A ESCREVER SERÁ BLOQUEADO(A)* ‼️\n\n⚠️ \`\`\`𝐒𝐄𝐆𝐔𝐍𝐃𝐀 𝐀𝐃𝐕𝐄𝐑𝐓𝐄𝐍𝐂𝐈𝐀\`\`\` ⚠️`, 
smsprivado2: (m) => `*@${m.sender.split`@`[0]} SERÁ BLOQUEADO(A) DO BOT. 😾 SE MENCIONO ANTERIORMENTE QUE NÃO PODÍA ESCREVER NO PRIVADO.*\n\n⚠️ \`\`\`𝐓𝐄𝐑𝐂𝐄𝐑𝐀 𝐀𝐃𝐕𝐄𝐑𝐓𝐄𝐍𝐂𝐈𝐀\`\`\` ⚠️`, 
...
smsNoSpam3: (m, motive) => `*@${m.sender.split`@`[0]} NO PODE USAR COMMANDOS DURANTE 1 MINUTO*\n\n*MOTIVO: ${motive}*`, 
smsNoSpam4: "SPAM DE MENSAGENS ALARMANTE", 
smsNoSpam5: (m, motive) => `*@${m.sender.split`@`[0]} NO PUEDE USAR COMMANDOS DURANTE 2 MINUTOS*\n\n*MOTIVO: ${motive}*`, 
smsNoSpam6: (mention) => `*${mention} ESTA PROIBIDO FAZER SPAM DE MENSAGENS!!*`, 

//antitraba.js
smsAntiTraba: (sender) => `O administrador @${m.sender.split("@")[0]} acaba de enviar UM texto que contém muitas características-.-!`, 
smsAntiTraba2: '[ ! ] detectei um mensagem que contém muitas características [ ! ]', 
smsAntiTraba3: 'Marcar o chat como lido ✓', 
smsAntiTraba4: (sender, name) => `o número : wa.me/${m.sender.split("@")[0]}\n• Alias : ${name}\n‼️Acaba de enviar um texto que contém muitas características que pode ocasionar falhas em seu dispositivos`, 
...
smsAutodetec4: (usuario, m, groupMetadata) => `🔒 ${usuario}*\n*𝙃𝘼 𝙋𝙀𝙍𝙈𝙄𝙏𝙄𝘿𝙊 𝙌𝙐𝙀 ${m.messageStubParameters[0] == 'on' ? '𝙎𝙊́ 𝘼𝘿𝙈𝙄𝙉𝙎' : '𝙏𝙊𝘿𝙊𝙎'} 𝙋𝙀𝘿𝙀 𝘾𝙊𝙉𝙁𝙄𝙂𝙐𝙍𝘼𝙍 ${groupMetadata.subject}*`, 
smsAutodetec5: (groupMetadata, usuario) => `*𝙊 𝙇𝙄𝙉𝙆 𝘿𝙊 ${groupMetadata.subject} 𝙁𝙊𝙄 𝙍𝙀𝙎𝙏𝘼𝙐𝙍𝘼𝘿𝙊 𝙋𝙊𝙍:*\n*» ${usuario}*`, 
smsAutodetec6: (m) => `𝙊 𝙂𝙍𝙐𝙋𝙊 *${m.messageStubParameters[0] == 'on' ? '𝙀𝙎𝙏𝘼 𝙁𝙀𝘾𝙃𝘼𝘿𝙊 🔒' : '𝙀𝙎𝙏𝘼 𝘼𝘽𝙀𝙍𝙏𝙊 🔓'}*\n ${m.messageStubParameters[0] == 'on' ? '𝙎𝙊́ 𝘼𝘿𝙈𝙄𝙉𝙄𝙎𝙏𝙍𝘼𝘿𝙊𝙍𝙀𝙎 𝙋𝙊𝘿𝙀𝙈 𝙀𝙎𝘾𝙍𝙀𝙑𝙀𝙍' : '𝙏𝙊𝘿𝙊𝙎 𝘼𝙂𝙊𝙍𝘼 𝙋𝙊𝘿𝙀𝙈 𝙀𝙎𝘾𝙍𝙀𝙑𝙀𝙍'} 𝙉𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊*`, 
smsAutodetec7: (m, usuario) =>  `@${m.messageStubParameters[0].split`@`[0]} 𝘼𝙂𝙊𝙍𝘼 𝙀́ 𝘼𝘿𝙈𝙄𝙉𝙄𝙎𝙏𝙍𝘼𝘿𝙊𝙍 𝙉𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊\n\n😼🫵𝘼𝘾̧𝘼̃𝙊 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝘿𝘼 𝙋𝙊𝙍: ${usuario}`, 
smsAutodetec8: (m,  usuario) => `@${m.messageStubParameters[0].split`@`[0]} 𝘿𝙀𝙄𝙓𝙊𝙐 𝘿𝙀 𝙎𝙀𝙍 𝘼𝘿𝙈𝙄𝙉𝙄𝙎𝙏𝙍𝘼𝘿𝙊𝙍 𝙉𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊\n\n😼🫵𝘼𝘾̧𝘼̃𝙊 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝘿𝘼 𝙋𝙊𝙍: ${usuario}`, 
...
//información
smsinfo: "💖 *Infórmate sobre as Novidades y recuerda tener la última versión.*", 
name: "𝙉𝙊𝙈𝙀", 
user: "𝙐𝙎𝙐𝘼𝙍𝙄𝙊(𝘼)", 
...
smsMalused7: '⚡ *USAR  COMANDO DESTA FORMA:*\n', 
smsMalused8: `🐈 *DEVE DE USAR O COMANDO COMO ESTE EXEMPLO:*\n`, 
smsMalused9: `🐈 *RESPONDE A UMA MENSAGEM CON O COMANDO O USE ESTE EXEMPLO:*\n`, 
...
//grupos
smsAdd: 'Hola! me apresento, sou GataBot-MD 🐈, sou um Bot de WhatsApp, uma pessoa do grupo utilizo o comando para añadirte al grupo, pero no pude agregarte, asi que te mando la invitacion para que te unas al grupo, te esperamos con ansias!!', 
smsAdd2: 'Enviando invitacion a seu privado...', 
smsGrup: '𝙏𝙊𝘿𝙊𝙎 𝙉𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊 𝘼𝙂𝙊𝙍𝘼 𝙋𝙊𝘿𝙀𝙈 𝙀𝙎𝘾𝙍𝙀𝙑𝙀𝙍!!', 
smaGrup2: '𝙎𝙊𝙈𝙀𝙉𝙏𝙀 𝘼𝘿𝙈𝙄𝙉𝙄𝙎𝙏𝙍𝘼𝘿𝙊𝙍𝙀𝙎 𝙋𝙊𝘿𝙀𝙈 𝙀𝙎𝘾𝙍𝙀𝙑𝙀𝙍 𝙉𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊!!', 
